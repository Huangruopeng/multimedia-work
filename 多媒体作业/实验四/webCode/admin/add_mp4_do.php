...
include("fileupload.class.php");  
    ...
    if($up->upload("pic")){
		...
		*/
		...
		$media=$file[0];
		$sql = "insert into media (type,title,name,media,content) values('$type','$title','$name','$media','$content')";  
		//die();		
		$result = $mysqli->query($sql); 
			if($result){  
				
				echo "<script>alert('添加成功！');window.location.href='list_mp4.php' </script>";  
			}else{  
